...
class Node:
    ...
    def __str__(self):
        ...
        root_label = "root " if self.is_root else ""
        node_label = f"{root_label}[feature={self.feature}, threshold={self.threshold}]\n"
        if self.left_child:
            node_label += self.left_child_add_prefix(str(self.left_child))
        if self.right_child:
            node_label += self.right_child_add_prefix(str(self.right_child))
        return node_label
