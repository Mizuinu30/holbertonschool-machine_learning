...
def create_padding_mask(seq):
    ...
    Returns:
        padding_mask: A tf.Tensor of shape (batch_size, 1, 1, seq_len)
                      where padded positions are 1 and non-padded positions are 0.
    """
...
def create_look_ahead_mask(size):
    """
    Creates a lookahead mask to prevent the decoder from attending to future tokens.

    Args:
        size: An integer representing the sequence length.

    Returns:
        look_ahead_mask: A tf.Tensor of shape (size, size), with 1s in the upper triangular
                         part and 0s in the lower part (including the diagonal).
    """
...
def create_masks(inputs, target):
    ...
    Returns:
        encoder_mask: Padding mask for the encoder (batch_size, 1, 1, seq_len_in).
        combined_mask: Lookahead and padding mask for the 1st attention block in the decoder
    ...
    # Ensures that the decoder doesn't attend to the padding tokens in the encoder's input
