...
class BayesianOptimization:
    ...
    def optimize(self, iterations=100):
        ...
        Returns:
            ...
            Y_opt: numpy.ndarray of shape (1,) representing the optimal function value.
        """
        for _ in range(iterations):
            # Get next sample location and its EI value
            X_next, EI = self.acquisition()

            # Check if the proposed point has already been sampled
            if np.any(np.abs(self.gp.X - X_next) <= 1e-8):
                print("Optimization stopped early: next point already sampled.")
                break

            # Sample the function at the proposed point
